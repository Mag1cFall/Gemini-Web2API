...
def get_firefox_cookies_manual():
    """
    Manually extracts __Secure-1PSID and __Secure-1PSIDTS cookies
    from Firefox's SQLite database.
    """
    print("Attempting to manually extract Gemini cookies from Firefox...")
    load_dotenv()
    
    # The path should point to your Firefox profile directory
    # e.g., C:\Users\YourUsername\AppData\Roaming\Mozilla\Firefox\Profiles\xxxxxxxx.default-release
    profile_path = os.getenv("FIREFOX_PROFILE_PATH")

    if not profile_path:
        print("\n--- CONFIGURATION ERROR ---")
        print("The 'FIREFOX_PROFILE_PATH' variable is not defined in your .env file.")
        print("Please add the path to your Firefox profile directory, for example:")
    ...
    if not cookies or len(cookies) < 2:
        ...
        print("Please ensure you are logged into https://gemini.google.com/ in Firefox and that Firefox is fully closed.")
        print("--------------\n")
        return None
